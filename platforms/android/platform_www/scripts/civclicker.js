...
function getGameSave() {
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
        ...
        fs.root.getFile("civ.txt", {create: false, exclusive: false}, function (fileEntry) {
            ...
            fileEntry.file(function (file) {
                ...
                reader.onloadend = function () {
                    ...
                    console.log("Loaded save game version " + saveVersion.major +
                        "." + saveVersion.minor + "." + saveVersion.sub + "(" + saveVersion.mod + ") via", "localStorage");
...
function getGameSettings() {
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
        console.log('file system open: ' + fs.name);
        fs.root.getFile("saveSettings.txt", {create: false, exclusive: false}, function (fileEntry) {
            console.log("fileEntry is file?" + fileEntry.isFile.toString());
            fileEntry.file(function (file) {
                var reader = new FileReader();
                reader.onloadend = function () {
...
function load(loadType) {
    ...
    if (loadType === "localStorage") {
        ...
        try {
            ...
            string1 = getGameSave();
            settingsString = getGameSettings();
            // window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
            //     console.log('file system open: ' + fs.name);
            //     fs.root.getFile("civ.txt", {create: false, exclusive: false}, function (fileEntry) {
            //         console.log("fileEntry is file?" + fileEntry.isFile.toString());
...
function save(savetype) {
    ...
    try {
        ...
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
            console.log('file system open: ' + fs.name);
            fs.root.getFile("civ.txt", {create: true, exclusive: false}, function (fileEntry) {
                console.log("fileEntry is file?" + fileEntry.isFile.toString());
                writeFile(fileEntry, JSON.stringify(saveVar));
            }, function (err) {
            });
        }, function (err) {
        });
        ...
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
            console.log('file system open: ' + fs.name);
            fs.root.getFile("saveSettings.txt", {create: true, exclusive: false}, function (fileEntry) {
                console.log("fileEntry is file?" + fileEntry.isFile.toString());
                writeFile(fileEntry, JSON.stringify(settingsVar));
            }, function (err) {
            });
        }, function (err) {
        });
...
function deleteSave() {
    ...
    //deleteCookie(saveTag);
    var message = "Save Deleted. Refresh page to start over?";
    try {
        //deleteCookie(saveTag2);
        ...
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
            console.log('file system open: f' + fs.name);
            fs.root.getFile("civ.txt", {create: true, exclusive: false}, function (fileEntry) {
                console.log("fileEntry is file?" + fileEntry.isFile.toString());
                writeFile(fileEntry, "");
            }, function (err) {
...
function gameLog(message) {
    ...
    if (ui.find("#logL").innerHTML != message) {
        ...
        while (--i > 1) {
            ui.find("#log" + i).innerHTML = ui.find("#log" + (i - 1)).innerHTML;
        }
...
setup.all = function () {
    ...
    document.addEventListener("deviceready", function (e) {
        deviceReady = true;
        if (contentLoaded) {
            let evento = new Event('everythingready');
    ...
    document.addEventListener("everythingready", function (e) {
        setup.events();
        setup.game();
        setup.loop();
        // Show the game
        ui.find("#main").style.display = "block";
        console.log("DEVICE IS READY CORDOVA");
    });
};
